...
public  url = 'http://localhost:8082/api/auth/';
//public url2='http://localhost:8081/signup'

  constructor(private httpClient: HttpClient, private localStoraqeService: LocalStorageService) {
  }

  register(registerPayload: RegisterPayload): Observable<any> {
    const token=  this.httpClient.post(this.url+'signup', registerPayload);
    console.log(token);
     this.localStoraqeService.store('authenticationToken', token);
      this.localStoraqeService.store('username', registerPayload.username);
      return token;
